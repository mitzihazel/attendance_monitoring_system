								...
								$check = mysql_fetch_assoc($attendance_check);
									...
									if(mysql_num_rows($attendance_check) > 0)
									{
										...
										if($check['timeOut'] == 0)
										{
											...
											$update = mysql_query("Update attendance_faculty set timeOut=NOW(),totalHour='".gmdate("H:i:s", $total)."',earlyOut='".gmdate("H:i:s", $earlys)."',image_out='$newname' where timeIn='".$check['timeIn']."' and subject='".$check['subject']."'");
										...
										} else 
										{
											...
											else 
											{
												...
												{
													echo "we";
													$add = mysql_query("INSERT INTO attendance_faculty(userID,subject,date,timeIn,image_in) VALUES('".$fetch['userID']."','".$fetch['subject_code']."',NOW(),NOW(),'$newname')");
													echo "<script> window.location.href='attendance-form.php'; </script>";
												}
												else
												{
													$add = mysql_query("INSERT INTO attendance_faculty(userID,subject,date,timeIn,tardinessCount,Attendance,image_in) VALUES('".$fetch['userID']."','".$fetch['subject_code']."',NOW(),NOW(),'".gmdate('H:i:s',$tardy)."','$late_check','$newname')");
													echo "<script> window.location.href='attendance-form.php'; </script>";
												}
										}
									}
									else
									{
										 echo "a";
											...
											else 
											{
												...
												$add = mysql_query("INSERT INTO attendance_faculty(userID,subject,date,timeIn,Attendance,image_in) VALUES('".$fetch['userID']."','".$fetch['subject_code']."',NOW(),NOW(),'$late_check','$newname')");
